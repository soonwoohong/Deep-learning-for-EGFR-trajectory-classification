...
This is a machine learning algorithm for trajectory-based classifcation written with keras packages.
...
from keras.layers import Dense, Conv2D, MaxPool2D, Flatten, Conv1D, MaxPooling1D
...
def get_model():
    ...
    stride=2
    model = Sequential()
    x = (Conv1D(input_shape=(max_length,2), filters=64, kernel_size=block_kernel, padding="same", activation="relu"))(inputs)
    x = (Conv1D(filters=64, kernel_size=block_kernel, padding="same", activation="relu"))(x)
    x = (MaxPooling1D(pool_size=pool, strides=stride))(x)
    x = (Conv1D(filters=128, kernel_size=block_kernel, padding="same", activation="relu"))(x)
    x = (Conv1D(filters=128, kernel_size=block_kernel, padding="same", activation="relu"))(x)
    x = (MaxPooling1D(pool_size=pool, strides=stride))(x)
    x = (Conv1D(filters=256, kernel_size=block_kernel, padding="same", activation="relu"))(x)
    x = (Conv1D(filters=256, kernel_size=block_kernel, padding="same", activation="relu"))(x)
    x = (Conv1D(filters=256, kernel_size=block_kernel, padding="same", activation="relu"))(x)
    x = (MaxPooling1D(pool_size=pool, strides=stride))(x)
    x = (Conv1D(filters=512, kernel_size=block_kernel, padding="same", activation="relu"))(x)
    x = (Conv1D(filters=512, kernel_size=block_kernel, padding="same", activation="relu"))(x)
    x = (Conv1D(filters=512, kernel_size=block_kernel, padding="same", activation="relu"))(x)
    x = (MaxPooling1D(pool_size=pool, strides=stride))(x)
    x = (Conv1D(filters=512, kernel_size=block_kernel, padding="same", activation="relu"))(x)
